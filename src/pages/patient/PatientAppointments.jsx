...
const PatientAppointments = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const prefillDoctorId = route?.params?.prefillDoctorId;
    if (prefillDoctorId) {
      fetchDoctors(prefillDoctorId);
      setDoctorSearch(prefillDoctorId);
    }
  }, [route?.params?.prefillDoctorId]);

  const handleDoctorSearch = (text) => {
    setDoctorSearch(text);
    if (text.trim()) {
      fetchDoctors(text.trim());
    } else {
      setFilteredDoctors(doctors);
    }
  };

  const handleDoctorQRScanned = (doctorId) => {
    setDoctorSearch(doctorId);
      ...
      <PatientLayout navigation={navigation} activeScreen="PatientAppointments">
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          ...
          <View style={styles.bookCard}>
            <Text style={styles.bookTitle}>Request Consultation</Text>

            <Text style={styles.label}>Find Doctor</Text>
            <View style={styles.searchRow}>
              <TextInput
                style={[styles.searchInput, { flex: 1 }]}
                placeholder="Search by name or doctor ID (e.g. DR-2026-00001)"
            ...
            >
              <Text style={styles.pickerBtnText}>
                {selectedDoctor
                  ? `${selectedDoctor.full_name} (${selectedDoctor.doctor_id})`
            ...
            <Text style={styles.label}>Concern (optional)</Text>
            <TextInput
            ...
            <TouchableOpacity
              ...
              disabled={booking}
            >
              <Text style={styles.bookBtnText}>{booking ? 'Sending...' : 'Send Request'}</Text>
            </TouchableOpacity>
          </View>
              ...
              <View style={styles.modalContent}>
                ...
                <ScrollView style={styles.doctorList}>
                  {filteredDoctors.length === 0 ? (
                    <Text style={styles.noDoctors}>No doctors found</Text>
                  ) : (
                    filteredDoctors.map(doc => (
                      ...
                      >
                        ...
                        <View style={styles.doctorInfo}>
                          ...
                          <Text style={styles.doctorSpec}>{doc.specialization || 'General'}</Text>
                          <Text style={styles.doctorId}>{doc.doctor_id}</Text>
                        </View>
            ...
            appointments.map(a => (
              <View key={a.id} style={styles.card}>
                <View style={styles.cardLeft}>
                  {a.patient_name && <Text style={styles.doctorNameCard}>{a.patient_name}</Text>}
...
const styles = StyleSheet.create({
  ...
  searchInput:     { backgroundColor: '#f4f6f8', borderRadius: 12, padding: 13, fontSize: 15, color: '#1a1a2e', borderWidth: 1, borderColor: '#e2e8f0' },
  ...
  doctorList:      { paddingBottom: 20 },
  doctorItem:      { flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  doctorItemSelected: { backgroundColor: '#f0f9ff' },
  doctorAvatar:    { width: 44, height: 44, borderRadius: 22, backgroundColor: '#E6F1FB', justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  doctorAvatarText: { color: '#185FA5', fontWeight: '800', fontSize: 16 },
  doctorInfo:      { flex: 1 },
  doctorName:      { fontSize: 15, fontWeight: '700', color: '#1a1a2e', marginBottom: 2 },
  doctorSpec:      { fontSize: 13, color: '#64748b', marginBottom: 2 },
  doctorId:        { fontSize: 12, color: '#185FA5', fontWeight: '600' },
  checkMark:       { fontSize: 20, color: '#22c55e' },
  noDoctors:       { textAlign: 'center', paddingVertical: 20, color: '#94a3b8' },

  sectionTitle:    { fontSize: 16, fontWeight: '700', color: '#1a1a2e', marginTop: 16, marginBottom: 10 },
  card:            { backgroundColor: '#fff', borderRadius: 14, padding: 16, marginBottom: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 1 },
  cardLeft:        { flex: 1 },
  doctorNameCard:  { fontSize: 15, fontWeight: '700', color: '#1a1a2e', marginBottom: 4 },
  date:            { fontSize: 14, fontWeight: '600', color: '#1a5c38' },
